...
export default function LocalData() {
  ...
  async function getValue(name, defaultValue) {
  ...
  async function retrieveData() {
    try {
      ...
      global.roscoreServerIp = String(
        await this.getValue('roscoreServerIp', DefaultConfig.roscoreServerIp()),
      );
      ...
      global.speedAuto = parseInt(
        await this.getValue('speedAuto', DefaultConfig.speedAuto()),
      );
      global.steerAuto = parseInt(
        await this.getValue('steerAuto', DefaultConfig.steerAuto()),
      );
      global.limitAuto = parseInt(
        await this.getValue('limitAuto', DefaultConfig.limitAuto()),
      );
      ...
      global.moveTime = parseInt(
        await this.getValue('moveTime', DefaultConfig.moveTime()),
      );
      global.stopTime = parseInt(
        await this.getValue('stopTime', DefaultConfig.stopTime()),
      );
      global.detectDistance = parseFloat(
        await this.getValue('detectDistance', DefaultConfig.detectDistance()),
      );
