...
export default (Src = {
  ...
  checkCorrectionMovements: function(corrections) {
    if (
      typeof parseInt(corrections) != "number" ||
      Number.isNaN(parseInt(corrections))
    ) {
  ...
  checkCorrectionFactor: function(correctionFactor) {
    if (
      typeof parseFloat(correctionFactor) != "number" ||
      Number.isNaN(parseInt(correctionFactor))
    ) {
  ...
  checkMoveTimeAuto: function(moveTime) {
    if (
      typeof parseInt(moveTime) != "number" ||
      Number.isNaN(parseInt(moveTime))
    ) {
      throw new Error("Andar por inválido.");
    }
    if (parseInt(moveTime) < 0 || parseInt(moveTime) > 10800) {
      //Evita números negativos ou superiores a 3 horas.
      throw new Error("Andar por inválido.");
    }
  },
  checkStopTimeAuto: function(stopTime) {
    if (
      typeof parseInt(stopTime) != "number" ||
      Number.isNaN(parseInt(stopTime))
    ) {
      throw new Error("Parar por inválido.");
    }

    if (parseInt(stopTime) < 0 || parseInt(stopTime) > 10800) {
      //Evita números negativos ou superiores a 3 horas.
      throw new Error("Parar por inválido.");
    }
  },
  checkDetectDistance: function(detectDistance) {
    if (
      typeof parseFloat(detectDistance) != "number" ||
      Number.isNaN(parseInt(detectDistance))
    ) {
    ...
    const buttons = [
      {
        ...
        onPress: () => {
          ...
          localData.storeData("correctionFactor", String(correctionFactor));
